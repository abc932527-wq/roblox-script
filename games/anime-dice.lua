-- VEIL HUB - ANIME DICE v3.1 (CLEAN & OPTIMIZED)
-- PlaceId: 113290951185459

local success, Library = pcall(function()
    return loadstring(game:HttpGet("https://raw.githubusercontent.com/abc932527-wq/roblox-script/main/ui-lib.lua"))()
end)

if not success then
    warn("[VEIL] Failed to load UI library")
    return
end
...
local success2, UI = pcall(function()
    return Library.new("[V] VEIL HUB", "Anime Dice")
end)

if not success2 then
    warn("[VEIL] Failed to create UI")
    return
end
...
UI:AddButton(rollTab, "Sell Bag Now", function()
    ...
    pcall(function() FireRemote("Sell", threshold) end)
...
UI:AddButton(miscTab, "Dump Remotes", function()
    ...
    UI:Notify("Dumped " .. tostring(#list) .. " remotes")
end)
...
-- REMOTE DISCOVERY (Silent, no console spam)
local Remotes = {}
...
-- MAIN LOOP (Optimized, no console spam)
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if S.AutoRoll and timers.roll >= 2.0 then
        timers.roll = 0
        pcall(function() FireRemote("Roll") end)
    end
    ...
    if S.AutoUpgradeUnits then
        pcall(function() FireRemote("Upgrade", "Unit") end)
    end
    if S.AutoUpgradeSkills then
        pcall(function() FireRemote("Upgrade", "Skill") end)
    end

    -- Auto Shop
    if S.AutoDiceShop then
        pcall(function() FireRemote("Shop", "Buy") end)
    end

    -- Auto Reroll
    if S.AutoTraitReroll then
        pcall(function() FireRemote("Reroll", "Trait", S.TargetUnit or "", S.TargetTrait or "Godly") end)
    end
    if S.AutoGradeReroll then
        pcall(function() FireRemote("Reroll", "Grade", S.TargetUnit or "", S.TargetGrade or "SSS") end)
    end

    -- Auto Tower
    if S.AutoTower then
        pcall(function() FireRemote("Tower", "Start", S.TargetTower or "Tower 1") end)
    end
    ...
    if timers.equip >= (S.EquipInterval or 15) then
        ...
        if S.AutoEquipBest then
            pcall(function() FireRemote("Equip", "BestUnit") end)
        end
        if S.AutoEquipBestDice then
            pcall(function() FireRemote("Equip", "BestDice") end)
        end
        if S.AutoEquipTeam then
            pcall(function() FireRemote("Equip", "BestTeam") end)
        end
    end
end)
